...
public class TestCaseSettingOption implements IAutocompleteOption, IKnownKeyword {
    ...
    public static List<TestCaseSettingOption> testCaseTableOptions = Arrays.asList(
            new TestCaseSettingOption("Documentation", "Used for specifying a test case documentation.", 1, 1000, -1),
            new TestCaseSettingOption("Tags", "Used for tagging test cases.", 1, 1000, -1),
            new TestCaseSettingOption("Arguments", "Used for specifying user keyword arguments.", 0, 1000, -1),
            new TestCaseSettingOption("Return", "Used for specifying user keyword return values.", 0, 1000, -1),
            new TestCaseSettingOption("Teardown", "The keyword in this setting will be executed in teardown mode at the end of this test case or user keyword.", 1, 1000, 0),
            ...
            new TestCaseSettingOption("Template", "Templates are currently not supported by Snowride!\n\nTest templates convert normal keyword-driven test cases into data-driven tests. Whereas the body of a keyword-driven test case is constructed from keywords and their possible arguments, test cases with template contain only the arguments for the template keyword. Instead of repeating the same keyword multiple times per test and/or with all tests in a file, it is possible to use it only per test or just once per file.\n" +
                    "\n" +
                    "Template keywords can accept both normal positional and named arguments, as well as arguments embedded to the keyword name. Unlike with other settings, it is not possible to define a template using a variable.", 1, 1000, 0),
    ...
    public static List<SettingsTableOption> settingsTableOptions = Arrays.asList(
            new SettingsTableOption("Library", "Test libraries are normally imported using the Library setting in the Setting table and having the library name in the subsequent column. Unlike most of the other data, the library name is both case- and space-sensitive. If a library is in a package, the full name including the package name must be used.\n" +
                    ...
                    "\n", 1, 2, -1),
            new SettingsTableOption("Resource", "Resource files are imported using the Resource setting in the Settings table. The path to the resource file is given in the cell after the setting name.\n" +
                    "\n" +
            ...
            new SettingsTableOption("Suite Setup", "Not only test cases but also test suites can have a setup and a teardown. A suite setup is executed before running any of the suite's test cases or child test suites, and a test teardown is executed after them. All test suites can have a setup and a teardown; with suites created from a directory they must be specified in a test suite initialization file.\n" +
                    ...
                    "\n", 1, 1000, 0),
            new SettingsTableOption("Suite Teardown", "Not only test cases but also test suites can have a setup and a teardown. A suite setup is executed before running any of the suite's test cases or child test suites, and a test teardown is executed after them. All test suites can have a setup and a teardown; with suites created from a directory they must be specified in a test suite initialization file.\n" +
                    "\n" +
                    "Similarly as with test cases, a suite setup and teardown are keywords that may take arguments. They are defined in the Setting table with Suite Setup and Suite Teardown settings, respectively. Keyword names and possible arguments are located in the columns after the setting name.\n" +
                    "\n" +
                    "If a suite setup fails, all test cases in it and its child test suites are immediately assigned a fail status and they are not actually executed. This makes suite setups ideal for checking preconditions that must be met before running test cases is possible.\n" +
                    ...
                    "The name of the keyword to be executed as a setup or a teardown can be a variable. This facilitates having different setups or teardowns in different environments by giving the keyword name as a variable from the command line.\n" +
                    "\n", 1, 1000, 0),
            new SettingsTableOption("Force Tags", "All test cases in a test case file with this setting always get specified tags. If it is used in the test suite initialization file, all test cases in sub test suites get these tags.", 1, 1000, -1),
            new SettingsTableOption("Default Tags", "Test cases that do not have a [Tags] setting of their own get these tags. Default tags are not supported in test suite initialization files.", 1, 1000, -1),
            new SettingsTableOption("Test Setup", "The easiest way to specify a setup or a teardown for test cases in a test case file is using the Test Setup and Test Teardown settings in the Setting table. ", 1, 1000, 0),
            new SettingsTableOption("Test Teardown", "The easiest way to specify a setup or a teardown for test cases in a test case file is using the Test Setup and Test Teardown settings in the Setting table. ", 1, 1000, 0),
            new SettingsTableOption("Test Template", "Test templates convert normal keyword-driven test cases into data-driven tests. Whereas the body of a keyword-driven test case is constructed from keywords and their possible arguments, test cases with template contain only the arguments for the template keyword. Instead of repeating the same keyword multiple times per test and/or with all tests in a file, it is possible to use it only per test or just once per file.\n" +
                    "\n" +
                    ...
                    "\n", 1, 1000, 0),
            new SettingsTableOption("Test Timeout", "The test case timeout can be set either by using the Test Timeout setting in the Setting table or the [Timeout] setting in the Test Case table. Test Timeout in the Setting table defines a default test timeout value for all the test cases in the test suite, whereas [Timeout] in the Test Case table applies a timeout to an individual test case and overrides the possible default value.\n" +
                    "\n" +
                    "Using an empty [Timeout] means that the test has no timeout even when Test Timeout is used. It is also possible to use value NONE for this purpose.\n" +
                    "\n" +
                    "Regardless of where the test timeout is defined, the first cell after the setting name contains the duration of the timeout. The duration must be given in Robot Framework's time format, that is, either directly in seconds or in a format like 1 minute 30 seconds. It must be noted that there is always some overhead by the framework, and timeouts shorter than one second are thus not recommended.\n" +
    ...
    protected TestCaseSettingOption(String setting, String documentation, int mandatoryArguments, int optionalArguments, int indexOfKeywordArgument) {
        this.setting = setting;
        this.documentation = documentation;
        this.mandatoryArguments = mandatoryArguments;
        this.optionalArguments = optionalArguments;
        this.indexOfKeywordArgument = indexOfKeywordArgument;
    }
    ...
    public String getInvariantName() {
        return Extensions.toInvariant(toString());
    }

    @Override
    public int getCompletionPriority() {
        return ExternalKeyword.PRIORITY_TEST_OPTION + toString().length();
    }
