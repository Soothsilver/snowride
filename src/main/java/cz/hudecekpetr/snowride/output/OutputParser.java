    ...
    public static void parseOutput(File outputXml) {
        ...
        try {
            ...
            if (!Settings.getInstance().disableOutputParsingWarning) {
                // Show warning in case output.xml was generated by Robot Framework older than 4.x
                String robotFrameworkVersion = StringUtils.substringBetween(result.getGenerator(), "Robot ", "(");
                if (!robotFrameworkVersion.startsWith("4")) {
                    Platform.runLater(() -> {
                        ...
                        SnowAlert alert = new SnowAlert(Alert.AlertType.WARNING,
                                "Unsupported output.xml format (Robot " + robotFrameworkVersion + "). Only version Robot 4.x is supported. " +
                    ...
                    if (highElement instanceof Scenario) {
                        ...
                        for (Keyword keyword : failingSetupTeardown) {
                            ...
                            if (findLineStartingWith(scenario.getLines(), toMatch, 1) == null) {
                                if (keyword.getType() == KeywordType.SETUP) {
                                    keyword.setType(KeywordType.TEST_SETUP);
                                } else if (keyword.getType() == KeywordType.TEARDOWN) {
